...
INTEGER CODING RULES:
  ...
  3. Has unpredictable behavior when shifting by an amount 
 ...
 */
int isPower2(int x) {
  // sign: to checks if x is negative or zero.
  int sign = (!(x >> 31)) & (!!x);
  // power: if x is power of 2, x & (x - 1) will be zero.
  int power = x & (x + (~1 + 1));
  return sign & (!power);
}
